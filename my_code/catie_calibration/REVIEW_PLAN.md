...
| X5 | **In-sample vs held-out.** Fit on Training, score once on EDA (no selection on EDA), report once on Test. Check which split each number was computed on, and whether any selection touched the split it is reported on. | Selection on the evaluation set biases every "held-out" claim. |
